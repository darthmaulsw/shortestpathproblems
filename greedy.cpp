...
vector<int> greedyBestFirstSearch(
    int start,
    int goal,
    ...
    open.push({start, heuristic(coords[start], coords[goal])});
    ...
    while (!open.empty()) {
        ...
        for (const Edge& e : graph[u]) {
            ...
            if (!visited[v]) {
                parent[v] = u;
                double h = heuristic(coords[v], coords[goal]);
                open.push({v, h});
            }
        }
    }
...
int main() {
    ...
    if (startNode == goalNode) {
        ...
        cout << "So the greedy search path is just that one node, and distance = 0.\n\n";
    }

    vector<int> path = greedyBestFirstSearch(startNode, goalNode, coords, graph);

    if (path.empty()) {
        cout << "No path found using greedy search.\n";
        return 0;
    }

    cout << "\nPath found (Greedy Best-First):\n";
    ...
    cout << "\nTotal path distance (sum of edges taken): " << totalDist << "\n";
